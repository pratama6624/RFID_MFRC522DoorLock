...
bool convertInputToUID(String input, byte *uid); 
...
bool convertInputToUID(String input, byte *uid) {
  ...
  for (byte i = 0; i < 7; i++) {
    String byteString = input.substring(i * 2, i * 2 + 2);
    uid[i] = (byte) strtol(byteString.c_str(), NULL, 16);
  }
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (convertInputToUID(inputUID, simulatedUID)) {
      // Setelah UID diinputkan, lakukan pemeriksaan validasi
        if (checkUID(simulatedUID, 7)) {
          Serial.println("Valid Card, turning GREEN LED on.");
          digitalWrite(GREEN_LED_PIN, HIGH);
          digitalWrite(RED_LED_PIN, LOW);
          tone(BUZZER_PIN, 1000);
          lcd.clear();
          lcd.print("Welcome Home Bro");
          delay(5000);
          lcd.clear();
          lcd.print("Scan card");
          digitalWrite(GREEN_LED_PIN, LOW);
